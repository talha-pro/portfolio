...
### 5. Graceful OpenAI quota/billing fallback (`app/api/chat/route.ts`)

**What it catches:** any failure of the actual OpenAI call inside `streamText` — most importantly a `429` with `error.code`/`error.type: "insufficient_quota"` in the response body (the account is out of credit or over its spending limit), but also a generic `429` rate-limit (different `error.code`, e.g. `rate_limit_exceeded`), other non-2xx API errors, and network-level failures (timeout, DNS, connection reset — these arrive as a plain `Error`/`TypeError`, not an `APICallError`). `classifyOpenAIError()` tells these apart by checking `APICallError.isInstance(err)`, `err.statusCode`, and parsing `err.responseBody` — **never** by status code alone, since a generic rate limit is also a 429.
- **Why this needed more than a try/catch:** the AI SDK never throws for these — a failed model call surfaces as a `{ type: "error" }` part inside `streamText`'s stream, not a rejected promise, so the route's existing outer `try/catch` (which still only covers request-parsing-level failures) can't see it. The route now builds its own `createUIMessageStream`, iterates `result.toUIMessageStream({ onError })` chunk-by-chunk, classifies+logs inside `onError`, and — when it sees the resulting `{ type: "error" }` chunk — **discards it** and writes a synthetic `start`/`text-start`/`text-delta`/`text-end`/`finish` sequence instead. The client (`useChat`) never sees an error state; the fallback text renders as a completely normal assistant message bubble, same as a real reply. (Verified against the real `ai`/`@ai-sdk/provider` packages with a mock model forced to throw the exact `insufficient_quota` shape OpenAI returns — the resulting SSE body contains no `error` chunk, only the fallback text.)
- **User-facing behavior is identical for every failure kind** (quota, generic rate limit, other API errors, network) — always the same fallback text: *"I'm temporarily unable to respond right now — please reach out to Talha directly via email or phone, he'd be happy to help."* (matches the tone of the system prompt's existing "not in my career info" fallback). Only the **server-side log line** and the **Slack alert condition** differ by kind.
- **If you see this fallback message showing up often** (in server logs as `OpenAI chat call failed (insufficient_quota): ...`, or as repeated Slack alerts) **it is not a bug** — it means the OpenAI account tied to `OPENAI_API_KEY` needs more credit or hit a spending limit. Check the OpenAI billing dashboard.
- **Slack alert:** reuses `SLACK_CHAT_WEBHOOK_URL` (no third webhook) — fires once, only for `insufficient_quota`, with a 1-hour in-memory cooldown (`QUOTA_ALERT_COOLDOWN_MS`, same best-effort/resets-on-cold-start tradeoff as the rate limiter above) so a burst of failed messages during an outage doesn't spam the channel.
